// --------------------------------------------------------------------------------------------------------------------
// <copyright file="CodeItemStatement.cs" company="">
//   
// </copyright>
// <summary>
//   Provides a wrapper for <see cref="Statement" /> objects so that they work
//   properly with the undo system and the ui.
// </summary>
// --------------------------------------------------------------------------------------------------------------------
namespace JaStDev.HAB.Designer
{
    ...
    public class CodeItemStatement : CodeItem, ICodeItemsOwner
    {
        ...
        /// <summary>Initializes a new instance of the <see cref="CodeItemStatement"/> class.</summary>
        /// <param name="toWrap">The to wrap.</param>
        /// <param name="isActive">The is active.</param>
        public CodeItemStatement(Statement toWrap, bool isActive)
        ...
        public override bool IsActive
        {
            ...
            set
            {
                if (value != base.IsActive)
                {
                    ...
                    if (fArguments != null)
                    {
                        // only pass along if there are agruments loaded.
        ...
        public CodeItemCollection Arguments
        {
            get
            {
                if (fArguments == null)
                {
                    // we leave null initially, so we can't have stack overflows because of a self referencing statement.
                    LoadArguments();
                }
            ...
            internal set
            {
                // don't check if fARguments != value -> when called, we always need to let the arguments list be refreshed, even if it already was null.
                fArguments = value;
        ...
        protected override void OnItemChanged(Neuron value)
        {
            ...
            Arguments = null;
                ...
                // we simply reset, so that it can be reloaded if required. Don't load all the arguments when initially set, this can cause a stack overflow if there is a self referencing item.
        }
        ...
        protected internal override void OutgoingLinkRemoved(Link link)
        {
            if (InternalChange == false)
            {
                ...
                {
                    Arguments = null; // we simply reset, so that it can be reloaded if required
                }
            }
        }
        ...
        {
            var iNew = ((Statement)Item).ArgumentsCluster;
            if (fArguments == null || iNew != fArguments.Cluster)
            {
                if (iNew != null)
                {
                    fArguments = new CodeItemCollection(this, iNew);
                }
                else
                {
                    fArguments = new CodeItemCollection(this, (ulong)PredefinedNeurons.Arguments);
                }
            }
        }
            ...
            WPF.Controls.CodePagePanel panel)
        {
            ...
            iNew.Element = new CtrlStatement();
            iNew.Data = this;
            return iNew;
        }
