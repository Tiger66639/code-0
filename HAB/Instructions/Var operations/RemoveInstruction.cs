// --------------------------------------------------------------------------------------------------------------------
// <copyright file="RemoveInstruction.cs" company="">
//   
// </copyright>
// <summary>
...
namespace JaStDev.HAB
{
    ...
    ///     ByRef 2+: the items that need to be added to the var.
    /// </summary>
    [NeuronID((ulong)PredefinedNeurons.RemoveInstruction)]
    public class RemoveInstruction : Instruction, IExecStatement
    {
        #region TypeOfNeuron
        ...
        public override ulong TypeOfNeuronID
        {
            get
            {
                return (ulong)PredefinedNeurons.RemoveInstruction;
            }
        }
        ...
        public bool Perform(Processor handler, System.Collections.Generic.IList<Neuron> args)
        {
                    ...
                    if (iList != null)
                    {
                        ...
                        for (var i = 1; i < args.Count; i++)
                        {
                            if (args[i] is Variable)
                            {
                                ...
                                foreach (var u in ((Variable)args[i]).ExtractValue(handler))
                                {
                                    iList.Remove(u);
                                }
                            }
                            else if (args[i] is ResultExpression)
                            {
                                ...
                                foreach (var u in SolveResultExpNoStackChange(args[i], handler))
                                {
                                    iList.Remove(u);
                                }
            ...
            else
            {
                ...
                LogService.Log.LogError("RemoveInstruction.Execute", "Invalid nr of arguments specified");
            }
        ...
        public override void Execute(Processor processor, System.Collections.Generic.IList<Neuron> args)
        {
            if (args != null && args.Count >= 2)
            {
                ...
                else
                {
                    LogService.Log.LogError(
                        "RemoveInstruction.Execute", 
                        string.Format("Can't remove, variable expected, found: {0}.", args[0]));
                }
            }
            else
            {
