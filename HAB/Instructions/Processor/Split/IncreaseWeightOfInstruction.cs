// --------------------------------------------------------------------------------------------------------------------
// <copyright file="IncreaseWeightOfInstruction.cs" company="">
//   
// </copyright>
// <summary>
...
namespace JaStDev.HAB
{
    ...
    ///         <item>
    ///             <description>
    ///                 An that defines how much the weight should be changed.
    ///             </description>
    ///         </item>
    ///     </list>
    /// </remarks>
    [NeuronID((ulong)PredefinedNeurons.IncreaseWeightOfInstruction)]
    public class IncreaseWeightOfInstruction : Instruction, IExecStatement
    {
        #region TypeOfNeuron
        ...
        public override ulong TypeOfNeuronID
        {
            get
            {
                return (ulong)PredefinedNeurons.IncreaseWeightOfInstruction;
            }
        }
        ...
        public bool Perform(Processor handler, System.Collections.Generic.IList<Neuron> args)
        {
            if (args != null && args.Count >= 2)
            {
                ...
                {
                    handler.SplitValues.IncreaseWeightOf(iFirst, iVal);
                }
                else
                {
                    ...
                    if (iTryVal.HasValue)
                    {
                        handler.SplitValues.IncreaseWeightOf(iFirst, iTryVal.Value);
                    }
                    else
                    {
                        LogService.Log.LogError("IncreaseWeightOf.Execute", "Second argument should be a double.");
                    }
                }
            }
            else
            {
                LogService.Log.LogError("IncreaseWeightOf.Execute", "No arguments specified");
            }
        ...
        public override void Execute(Processor processor, System.Collections.Generic.IList<Neuron> args)
        {
            if (args != null && args.Count >= 2)
            {
                ...
                {
                    processor.SplitValues.IncreaseWeightOf(args[0], iVal);
                }
            }
            else
            {
        ...
        private bool GetVal(Neuron neuron, out double result)
        {
            ...
            LogService.Log.LogError("IncreaseWeightOf.Execute", "IntNeuron or DoubleNeuron expected as arg.");
