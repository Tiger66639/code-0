// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SplitFixedInstruction.cs" company="">
//   
// </copyright>
// <summary>
//   <para>
//   Splits up the processor into identical sub processors which continue
//   processing from the same point. The second argument determins the nr of
//   processors that will be created. Each processor gets an even nr of items
//   from the splitlist.
//   </para>
//   <para>
//   When the split is finished, all results are combined. If multiple
//   processors have the same result, that result will only be included 1 time
//   and receive the maximum weight that it had accross all processors. Whe a
...
namespace JaStDev.HAB
{
    ...
    ///     (executed) when the split is ready. 2: the nr of threads/processors to
    ///     create 3: the var that will contain the result of the split (must be
    ///     declared as byref, or as the content of another var). 4: a list of
    ///     neurons on which to perform the split (for each item, a new sub processor
    ///     will be created and in that processor, only
    /// </remarks>
    [NeuronID((ulong)PredefinedNeurons.SplitFixedInstruction)]
    public class SplitFixedInstruction : Instruction, IExecStatement
    {
        #region TypeOfNeuron
        ...
        /// </summary>
        /// <value>
        ///     <see cref="JaStDev.HAB.PredefinedNeurons.SplitFixedInstruction" /> .
        /// </value>
        public override ulong TypeOfNeuronID
        {
            get
            {
                return (ulong)PredefinedNeurons.SplitFixedInstruction;
            }
        }
            ...
            if (args.Count >= 3)
            {
                ...
                try
                {
                    ...
                    if (args.Count > i)
                    {
                        ...
                        {
                            LogService.Log.LogError("SplitFixedInstruction.Execute", "Can't split, Invalid weight.");
                        }
                    }
                    else
                    {
                        LogService.Log.LogError("SplitFixedInstruction.Execute", "Invalid nr of arguments specified");
                    }
                }
                finally
                {
        ...
        /// <param name="processor">The processor.</param>
        /// <param name="nrThreads">The nr threads.</param>
        /// <param name="args">The args.</param>
        /// <param name="startFrom">The start from.</param>
        private static void DoSplit(
            Processor processor, 
            int nrThreads, System.Collections.Generic.IList<Neuron> args, 
            int startFrom)
        {
                ...
                if (args.Count > startFrom)
                {
                    ...
                    if (iCode != null)
                    {
                        ...
                        if (iVar != null)
                        {
                            ...
                            iSplitArgs.ThreadCount = nrThreads;
