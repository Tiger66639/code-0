// --------------------------------------------------------------------------------------------------------------------
// <copyright file="IsClusteredByAnyInstruction.cs" company="">
//   
// </copyright>
// <summary>
...
// </summary>
// --------------------------------------------------------------------------------------------------------------------
namespace JaStDev.HAB.Instructions.Contains
{
    /// <summary>
    ///     Returns <see langword="true" /> if the first argument, is clustered by all
    ///     the following argumetns, which should be clusters, otherwise,
    ...
    /// </remarks>
    [NeuronID((ulong)PredefinedNeurons.IsClusteredByAnyInstruction)]
    public class IsClusteredByAnyInstruction : SingleResultInstruction, ICalculateBool
    {
        #region TypeOfNeuron
        ...
        public override ulong TypeOfNeuronID
        {
            get
            {
                return (ulong)PredefinedNeurons.IsClusteredByAnyInstruction;
            }
        }
        ...
        public bool CalculateBool(Processor processor, System.Collections.Generic.IList<Neuron> list)
        {
            if (list.Count >= 1)
            {
                    ...
                    if (iToCheck.ClusteredByIdentifier != null)
                    {
                        ...
                        try
                        {
                            for (var i = 1; i < list.Count; i++)
                            {
                                ...
                                if (iItem != null)
                                {
                                    if (iClustered.ContainsUnsafe(iItem))
                                    {
                                        return true;
                                    }
                                }
                ...
                LogService.Log.LogError(
                    "IsClusteredByInstruction.InternalGetValue", 
