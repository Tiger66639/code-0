// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ContainsChildrenInstruction.cs" company="">
//   
// </copyright>
// <summary>
//   Returns <see langword="true" /> if the first argument (which must be a
//   cluster), contains any the following argumetns, otherwise,
...
namespace JaStDev.HAB
{
    ...
    /// </remarks>
    [NeuronID((ulong)PredefinedNeurons.ContainsChildrenInstruction)]
    public class ContainsChildrenInstruction : SingleResultInstruction, ICalculateBool
    {
        #region TypeOfNeuron
        ...
        public override ulong TypeOfNeuronID
        {
            get
            {
                return (ulong)PredefinedNeurons.ContainsChildrenInstruction;
            }
        }
            ...
            if (list.Count >= 1)
            {
                    ...
                    if (list.Count > 1)
                    {
                        ...
                        try
                        {
                            for (var i = 1; i < list.Count; i++)
                            {
                                ...
                                if (iItem != null)
                                {
                                    if (iChildren.ContainsUnsafe(iItem))
                                    {
                                        return true;
                                    }
                                }
                ...
                if (Settings.LogContainsChildrenNoCluster)
                {
                    LogService.Log.LogError(
                        "ContainsChildrenInstruction.InternalGetValue", 
