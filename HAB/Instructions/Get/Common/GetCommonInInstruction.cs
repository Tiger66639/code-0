// --------------------------------------------------------------------------------------------------------------------
// <copyright file="GetCommonInInstruction.cs" company="">
//   
// </copyright>
// <summary>
//   Returns all the neurons that link to the specified neurons. Each
//   clustered that is returned, must own all the arguments as children (order
//   is free, but can further be specified in the filter part).
// </summary>
// --------------------------------------------------------------------------------------------------------------------
namespace JaStDev.HAB
{
    ...
    /// </remarks>
    [NeuronID((ulong)PredefinedNeurons.GetCommonInInstruction)]
    internal class GetCommonInInstruction : MultiResultInstruction
    {
        #region TypeOfNeuron
        ...
        public override ulong TypeOfNeuronID
        {
            get
            {
                return (ulong)PredefinedNeurons.GetCommonInInstruction;
            }
        }
        ...
        public override void GetValues(Processor processor, System.Collections.Generic.IList<Neuron> list)
        {
            if (list != null)
            {
                ...
                var iLock = BuildLinksInLock(list, false);
                LockManager.Current.RequestLocks(iLock);
                try
                {
                    iCommmons = GetCommonInUnsafeUlong(list);
                }
                finally
                {
            ...
            {
                LogService.Log.LogError("GetCommonInInstruction.InternalGetValue", "Invalid nr of arguments specified");
            }
        }
    }
}