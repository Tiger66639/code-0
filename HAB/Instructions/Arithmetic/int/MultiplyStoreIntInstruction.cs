// --------------------------------------------------------------------------------------------------------------------
// <copyright file="MultiplyStoreIntInstruction.cs" company="">
//   
// </copyright>
// <summary>
...
namespace JaStDev.HAB
{
    ...
    ///     store the result in an int.
    /// </summary>
    [NeuronID((ulong)PredefinedNeurons.MultiplyStoreIntInstruction)]
    public class MultiplyStoreIntInstruction : Instruction, IExecStatement
    {
        #region TypeOfNeuron
        ...
        public override ulong TypeOfNeuronID
        {
            get
            {
                return (ulong)PredefinedNeurons.MultiplyStoreIntInstruction;
            }
        }
        ...
        public bool Perform(Processor handler, System.Collections.Generic.IList<Neuron> list)
        {
            if (list.Count == 3)
            {
                ...
                if (iRes == null)
                {
                    LogService.Log.LogWarning("MultiplyStoreInt", "first arg should be an int");
                    return true;
                }
                ...
                try
                {
                    if (CalculateInt(handler, list[1], out iSecond) == false)
                    {
                        ...
                        {
                            LogService.Log.LogWarning("MultiplyStoreInt", "second arg should be an int");
                            return true;
                        }
                    }
                    ...
                    if (CalculateInt(handler, list[2], out iThird) == false)
                    {
                        ...
                        {
                            LogService.Log.LogWarning("MultiplyStoreInt", "third arg should be an int");
                            return true;
                        }
                    }
        ...
        public override void Execute(Processor processor, System.Collections.Generic.IList<Neuron> args)
        {
            ...
            {
                LogService.Log.LogError("MultiplyStoreInt", "Invalid nr of arguments.");
            }
        }
    }
}