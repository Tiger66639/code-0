// --------------------------------------------------------------------------------------------------------------------
// <copyright file="MinusStoreDoubleInstruction.cs" company="">
//   
// </copyright>
// <summary>
//   Returns the / of all the arguments, which must either be doubles.
// </summary>
// --------------------------------------------------------------------------------------------------------------------
namespace JaStDev.HAB
{
    ...
    ///     </list>
    /// </remarks>
    [NeuronID((ulong)PredefinedNeurons.MinusStoreDoubleInstruction)]
    public class MinusStoreDoubleInstruction : Instruction, IExecStatement
    {
        #region TypeOfNeuron
        ...
        public override ulong TypeOfNeuronID
        {
            get
            {
                return (ulong)PredefinedNeurons.MinusStoreDoubleInstruction;
            }
        }
        ...
        public bool Perform(Processor handler, System.Collections.Generic.IList<Neuron> list)
        {
            if (list.Count == 3)
            {
                ...
                if (iRes == null)
                {
                    LogService.Log.LogWarning("MinusStoreDouble", "first arg should be a double");
                    return true;
                }
                ...
                try
                {
                    if (CalculateDouble(handler, list[1], out iSecond) == false)
                    {
                        ...
                        {
                            LogService.Log.LogWarning("MinusStoreDouble", "second arg should be an int or double");
                            return true;
                        }
                    }
                    ...
                    if (CalculateDouble(handler, list[2], out iThird) == false)
                    {
                        ...
                        {
                            LogService.Log.LogWarning("MinusStoreDouble", "third arg should be an int or double");
                            return true;
                        }
                    }
