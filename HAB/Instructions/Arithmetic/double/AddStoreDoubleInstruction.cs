// --------------------------------------------------------------------------------------------------------------------
// <copyright file="AddStoreDoubleInstruction.cs" company="">
//   
// </copyright>
// <summary>
...
namespace JaStDev.HAB.Instructions.Arithmetic
{
    ...
    ///     store the result in a double.
    /// </summary>
    [NeuronID((ulong)PredefinedNeurons.AddStoreDoubleInstruction)]
    public class AddStoreDoubleInstruction : Instruction, IExecStatement
    {
        #region TypeOfNeuron
        ...
        public override ulong TypeOfNeuronID
        {
            get
            {
                return (ulong)PredefinedNeurons.AddStoreDoubleInstruction;
            }
        }
        ...
        public bool Perform(Processor handler, System.Collections.Generic.IList<Neuron> list)
        {
            if (list.Count == 3)
            {
                ...
                if (iRes == null)
                {
                    LogService.Log.LogWarning("AddStoreDouble", "first arg should be a double");
                    return true;
                }
                ...
                try
                {
                    if (CalculateDouble(handler, list[1], out iSecond) == false)
                    {
                        ...
                        {
                            LogService.Log.LogWarning("AddStoreDouble", "second arg should be an int or double");
                            return true;
                        }
                    }
                    ...
                    if (CalculateDouble(handler, list[2], out iThird) == false)
                    {
                        ...
                        {
                            LogService.Log.LogWarning("AddStoreDouble", "third arg should be an int or double");
                            return true;
                        }
                    }
